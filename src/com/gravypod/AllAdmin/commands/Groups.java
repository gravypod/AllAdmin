...
public class Groups extends CommandUtil {
	...
	public boolean doCommand(final CommandSender sender, final Command command, final String cmd, final String[] args) {
		...
		switch(arg) {
			...
			case flag:
				...
				if (((AllAdminUser) fromUser).getGroup().getPermissions().contains("edit_flags_" + args[1])) {
					boolean tempValue;
					if (tempValue = Boolean.parseBoolean(args[3])) {
						...
						if (((AllAdminUser) fromUser).getGroup().hasFlag(args[2])) {
							...
							if (playerGroup.getFlagBoolean(args[2])) {
								playerGroup.removeFlag(args[2]);
								playerGroup.addFlag(args[2], tempValue);
								return true;
							}
						}
					}
	...
	private void incorrectUsage(final CommandSender sender) {
		...
		sender.sendMessage("You can use it correctly like this: " + commandHelp());
	}
